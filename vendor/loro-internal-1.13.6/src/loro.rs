...
use crate::{
    ...
    dag::{Dag, DagUtils},
    diff_calc::{DiffCalculator, DiffMode, ExternalImportDiff},
    encoding::{
...
impl LoroDoc {
    ...
    pub fn new() -> Self {
        ...
        Self::from_oplog(oplog, visible_op_count)
    }

    fn from_oplog(oplog: OpLog, visible_op_count: Arc<AtomicUsize>) -> Self {
    ...
    /// Open a document whose complete causal history is kept in an injected
    /// external change store.
    ///
    /// A non-empty store requires the state checkpoint returned by the same
    /// [`LoroDoc::flush_external_store`] publication. The checkpoint contains
    /// the digest of the exact canonical store metadata; stale, newer, mixed,
    /// missing, or corrupt pairs are rejected rather than replayed. The caller
    /// must authenticate and publish the store root and checkpoint atomically.
    /// Neither the checkpoint nor store metadata is trusted alone. Reopen reads
    /// compact causal and complete owner-bound tracker commitments, not
    /// historical text or causal blocks.
    pub fn from_external_store(
        checkpoint: Option<&[u8]>,
        external_kv: KvStoreHandle,
    ) -> LoroResult<Self> {
        let checkpoint = checkpoint
            .map(crate::external_store::decode_checkpoint)
            .transpose()?;
        let visible_op_count = Arc::new(AtomicUsize::new(0));
        let oplog = OpLog::new_external(visible_op_count.clone(), external_kv)?;
        if !oplog.is_empty() && checkpoint.is_none() {
            return Err(LoroError::ArgErr(
                "a non-empty external change store requires a state checkpoint".into(),
            ));
        }

        let doc = Self::from_oplog(oplog, visible_op_count);
        if let Some(checkpoint) = checkpoint {
            doc.with_barrier(|| doc.restore_external_store_checkpoint(checkpoint))?;
        }
        Ok(doc)
    }

    fn restore_external_store_checkpoint(
        &self,
        checkpoint: crate::external_store::StateCheckpoint,
    ) -> LoroResult<()> {
        let mut oplog = self.oplog.lock();
        oplog.probe_external_store()?;
        if oplog.is_shallow() {
            return Err(LoroError::ArgErr(
                "external change-store checkpoints require complete history".into(),
            ));
        }
        if checkpoint.metadata_digest != oplog.external_metadata_digest()?
            || checkpoint.frontiers != *oplog.frontiers()
        {
            return Err(LoroError::DecodeError(
                "external checkpoint does not match store metadata".into(),
            ));
        }
        let mut state = self.state.lock();
        state.check_before_decode_snapshot()?;
        if let Some(error) = oplog.take_external_store_error() {
            return Err(error);
        }

        state.store.decode(checkpoint.state)?;
        state.init_with_states_and_version(
            checkpoint.frontiers.clone(),
            &oplog,
            vec![],
            false,
            Default::default(),
        )?;
        let text_containers = state
        ...
        if let Some(error) = self.oplog.lock().take_external_store_error() {
            return Err(error);
        }
        Ok(())
    }
    ...
    fn _import_with(
        ...
        self.oplog.lock().probe_external_store()?;
    ...
    pub(crate) fn import_changes_and_apply_delta_to_state_if_needed(
        ...
        let mut oplog = self.oplog.lock();
        let mut arena_checkpoint = Some(oplog.arena.checkpoint_for_rollback());
        let changes = match decode_changes(&mut oplog) {
            Ok(changes) => changes,
            Err(e) => {
                oplog.arena.rollback(arena_checkpoint.take().unwrap());
                return Err(e);
            }
        };

        let preflight = oplog.preflight_import_changes(&changes);
        if let Err(error) = oplog.validate_external_import_changes(&changes) {
            oplog.arena.rollback(arena_checkpoint.take().unwrap());
            return Err(error);
        }
        if let Err(error) = oplog.validate_external_import_dependencies(&changes) {
            oplog.arena.rollback(arena_checkpoint.take().unwrap());
            return Err(error);
        }
        let external_import = oplog.change_store().is_external();
        ...
        if self.is_detached() {
            if external_import {
                oplog.begin_import_rollback_with_arena(arena_checkpoint.take().unwrap());
            }
        ...
        let old_vv = oplog.vv().clone();
        let old_frontiers = oplog.frontiers().clone();
        let rollback_enabled = preflight.needs_state_apply_rollback || external_import;
        if rollback_enabled {
            oplog.begin_import_rollback_with_arena(arena_checkpoint.take().unwrap());
        }
        ...
        if &old_vv != oplog.vv() {
            let external_diff = if external_import {
                match oplog.prepare_external_import_diff(
                    &old_vv,
                    &old_frontiers,
                    oplog.vv(),
                    oplog.dag.get_frontiers(),
            ...
            let (diff, diff_mode, external_baseline) = match external_diff {
                Some(ExternalImportDiff::Fast { diff, baseline }) => {
                    (diff, DiffMode::Import, Some(baseline))
                }
                None => {
                    let mut diff = DiffCalculator::new(false);
                    let (diff, mode) = diff.calc_diff_internal(
                        &oplog,
                        &old_vv,
                        &old_frontiers,
                        oplog.vv(),
            ...
            if let Some(baseline) = external_baseline {
                oplog.commit_external_import_baseline(baseline);
            }
        }
    ...
    /// Flush external change blocks and return a state checkpoint paired to
    /// the exact resulting store metadata. The caller must authenticate and
    /// publish that checkpoint and store root atomically. Successor publication
    /// is authorized from the exact predecessor metadata digest loaded with the
    /// caller-authenticated checkpoint.
    pub fn flush_external_store(&self) -> LoroResult<Vec<u8>> {
        self.with_barrier(|| {
            let mut oplog = self.oplog.lock();
            let metadata_digest = oplog.flush_external_change_store()?;
            let mut state = self.state.lock();
            state.ensure_all_alive_containers();
            let frontiers = state.frontiers.clone();
            let state = state.store.encode();
            crate::external_store::encode_checkpoint(&frontiers, state, metadata_digest)
        })
    }

    /// Flush and evict parsed change blocks, DAG nodes, and history caches.
    /// Visible document state remains materialized.
    pub fn evict_external_store_cache(&self) -> LoroResult<()> {
        self.with_barrier(|| self.oplog.lock().evict_external_change_store_cache())
    }

    /// Return and clear a sticky external change-store error.
    pub fn take_external_store_error(&self) -> Option<LoroError> {
        self.oplog.lock().take_external_store_error()
    }
...
mod test {
    ...
    use crate::{
        ...
        kv_store::{kv_store_handle, KvStore, KvStoreHandle},
        loro::ExportMode,
    ...
    #[derive(Clone, Debug)]
    struct LatePhysicalFaultStore {
        inner: Arc<LatePhysicalFaultStoreInner>,
    }

    #[derive(Debug, Default)]
    struct LatePhysicalFaultStoreInner {
        data: StdMutex<BTreeMap<Vec<u8>, Bytes>>,
        scan_calls: AtomicUsize,
        fault_call: AtomicUsize,
        fault_mode: AtomicUsize,
    }

    impl LatePhysicalFaultStore {
        fn new() -> (Self, KvStoreHandle) {
            let store = Self {
                inner: Arc::new(LatePhysicalFaultStoreInner::default()),
            };
            (store.clone(), kv_store_handle(store))
        }

        fn change_blocks(&self) -> Vec<(Vec<u8>, Bytes)> {
            self.inner
                .data
                .lock()
                .unwrap()
        ...
        fn restore_change_blocks(&self, blocks: &[(Vec<u8>, Bytes)]) {
            let mut data = self.inner.data.lock().unwrap();
            data.retain(|key, _| key.len() != 12);
            data.extend(blocks.iter().cloned());
        }

        fn arm(&self, scan_call: usize, corrupt: bool) {
            self.inner.scan_calls.store(0, Ordering::SeqCst);
            self.inner
                .fault_mode
                .store(if corrupt { 2 } else { 1 }, Ordering::SeqCst);
            self.inner.fault_call.store(scan_call, Ordering::SeqCst);
        }

        fn disarm(&self) {
            self.inner.fault_call.store(0, Ordering::SeqCst);
            self.inner.fault_mode.store(0, Ordering::SeqCst);
        }
    ...
    impl KvStore for LatePhysicalFaultStore {
        ...
        fn scan(
            ...
            let call = self.inner.scan_calls.fetch_add(1, Ordering::SeqCst) + 1;
            let mut data = self.inner.data.lock().unwrap();
            if self.inner.fault_call.load(Ordering::SeqCst) == call {
                let target = data
                    .keys()
                    .filter(|key| {
                        key.len() == 12 && key_is_within_bounds(key.as_slice(), &start, &end)
                    })
                    .next_back()
                    .cloned();
                if let Some(target) = target {
                    match self.inner.fault_mode.load(Ordering::SeqCst) {
                        1 => {
                            data.remove(&target);
                        }
                        2 => {
                            let value = data.get_mut(&target).unwrap();
                            *value = value.slice(..value.len().max(2) / 2);
                        }
                        _ => {}
                    }
                }
            ...
            let rows = data
                .iter()
                .filter(|(key, _)| key_is_within_bounds(key, &start, &end))
        ...
        fn export_all(&mut self) -> Bytes {
            panic!("late physical fault store is not exported")
        }

        fn import_all(&mut self, _bytes: Bytes) -> Result<(), String> {
            Err("late physical fault store is not imported".to_string())
        }
    ...
    fn key_is_within_bounds(key: &[u8], start: &Bound<&[u8]>, end: &Bound<&[u8]>) -> bool {
    ...
    #[test]
    fn late_physical_change_block_fault_rolls_back_and_retries() {
        for corrupt in [false, true] {
            let (store_control, store) = LatePhysicalFaultStore::new();
            let doc = LoroDoc::from_external_store(None, store).unwrap();
            doc.start_auto_commit();
            doc.set_peer_id(1).unwrap();
            doc.get_map("map").insert("base", "b".repeat(1024)).unwrap();
            doc.get_text("text")
                .insert(0, "base", PosType::Unicode)
                .unwrap();
            doc.commit_then_renew();
            let base_vv = doc.oplog_vv();
            let base_updates = doc.export(ExportMode::all_updates()).unwrap();
            ...
            incoming
                .get_map("map")
                .insert("incoming", "old-base")
                .unwrap();
            incoming
                .get_text("text")
                .insert(4, "-incoming", PosType::Unicode)
                .unwrap();
            incoming.commit_then_renew();
            let incoming_update = incoming.export(ExportMode::updates(&base_vv)).unwrap();

            doc.set_peer_id(3).unwrap();
            for index in 0..32 {
                doc.get_map("map")
                    .insert(&format!("resident-{index:02}"), index as i64)
                    .unwrap();
                doc.set_next_commit_message(&format!("resident-{index:02}"));
                doc.commit_then_renew();
            }
            doc.get_text("text")
                .insert(4, "-resident", PosType::Unicode)
                .unwrap();
            doc.commit_then_renew();

            // Persist the resident blocks without advancing the import baseline.
            // This intentionally reaches the post-journal baseline catch-up read.
            {
                ...
                oplog
                    .change_store()
                    .try_flush_and_compact(&vv, &frontiers)
                    .unwrap();
                oplog.change_store().evict_parsed_cache();
            }
            let blocks = store_control.change_blocks();
            assert!(blocks.len() > 1);
            ...
            doc.oplog.lock().change_store().evict_parsed_cache();
            let before_vv = doc.oplog_vv();
            let before_state = doc.get_deep_value();

            store_control.arm(2, corrupt);
            let result = doc.import(&incoming_update);
            let scan_calls = store_control.scan_calls();
            let error = match result {
                Ok(_) => panic!("late physical fault missed after {scan_calls} scans"),
                Err(error) => error,
            };
            assert!(
                scan_calls >= 2,
                "the physical fault must follow dependency preflight"
            );
            assert!(
                error.to_string().contains("missing") || error.to_string().contains("corrupt"),
                "unexpected late physical block error: {error}"
            );
            assert_eq!(doc.oplog_vv(), before_vv);
            assert_eq!(doc.get_deep_value(), before_state);

            store_control.disarm();
            store_control.restore_change_blocks(&blocks);
            doc.oplog.lock().change_store().evict_parsed_cache();
            assert!(doc.import(&incoming_update).unwrap().pending.is_none());
            assert_eq!(doc.get_deep_value(), expected.get_deep_value());
            assert_eq!(doc.oplog_vv(), expected.oplog_vv());
        }
    }
