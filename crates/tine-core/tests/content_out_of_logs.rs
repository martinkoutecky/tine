...
const ALLOWLIST: &[AllowedSite] = &[
    ...
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[1234], macro_name: "eprintln", bucket: "d", class: "fixed-family-report", why: "report_projection_failure's always-on line is the fixed failure family and no error value", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[1236], macro_name: "eprintln", bucket: "b", class: "directed-core-detail", why: "report_projection_failure's detail line repeats the family with the raw error, for a directed investigation", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[1247], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "projection directory creation carries only a std::io::Error, whose Display never includes the path", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/direct_projection.rs", lines: &[1265], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "projection lease acquisition carries only a std::io::Error, whose Display never includes the path", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/model.rs", lines: &[15078], macro_name: "eprintln", bucket: "d", class: "numeric-shape", why: "isolated search worker panic reports only a worker number", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/model.rs", lines: &[20768, 23652], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "reconcile and isolated-parse failures contain no path, title, content, or raw error", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/model.rs", lines: &[22519, 22708], macro_name: "eprintln", bucket: "a", class: "fixed-debug", why: "Guide-page refusal messages are fixed literals", gate: "cfg(debug_assertions)" },
    ...
    AllowedSite { file: "crates/tine-core/src/oplog/sqlite.rs", lines: &[1940, 4627, 4631, 4638, 4651, 4663, 4671, 4683, 5445], macro_name: "eprintln", bucket: "b", class: "directed-core-trace", why: "SQLite construction diagnostics run only under explicit trace flags", gate: "TINE_PHASE_TRACE/TINE_TERMINAL_TRACE" },
    AllowedSite { file: "crates/tine-core/src/publish.rs", lines: &[4593, 4623], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "publication refusals report only a fixed shape or collision count", gate: "runtime_debug_diagnostics_enabled" },
    ...
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[7192, 7210], macro_name: "eprintln", bucket: "a", class: "numeric-debug", why: "clean-open stage and counter reports contain fixed names and numeric measurements", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[7387, 7391, 7417, 7429], macro_name: "eprintln", bucket: "d", class: "content-free-error", why: "disposable checkpoint fallbacks carry typed store/decode errors describing batch ids, sealed kinds and sequences", gate: "always-on reviewed failure" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[7565, 21536, 21797, 22647, 22687, 22719], macro_name: "eprintln", bucket: "a", class: "content-free-debug", why: "receipt, pending-projection, and conflict-resolution reports contain only counts or fixed states", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[21207, 21298, 21364], macro_name: "eprintln", bucket: "b", class: "directed-core-detail", why: "foreground mutation detail is available only for explicitly enabled runtime debugging", gate: "runtime_debug_diagnostics_enabled" },
    AllowedSite { file: "crates/tine-core/src/sync_runtime.rs", lines: &[22179], macro_name: "eprintln", bucket: "b", class: "numeric-trace", why: "actor tick report contains a fixed branch label, duration, and pending count", gate: "TINE_TICK_TRACE" },
