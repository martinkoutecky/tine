...
- **Android root Back now verifies managed storage can stop before requesting activity exit.** If that safety check refuses, Tine stays editable and a later Back retries the complete close. Once the check succeeds, Tine remains protected even if Android declines the activity exit; the next Back retries only that exit handoff.
