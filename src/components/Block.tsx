...
function SheetContainer(props: { children: JSX.Element }): JSX.Element {
  ...
  const measure = () => {
    ...
    if (main) {
      ...
      // Center within the main pane's gutter-to-gutter span, clamped so the
      // sheet never pokes past either gutter (a parent-centered sheet drifts
      // right by half the outline indent and overflows the pane).
  ...
  // Absolute rects can be sampled while the page is still translating (a
  // position-only settle fires no ResizeObserver), leaving a breakout sheet
  // parked off-center with nothing to re-trigger a measure. Closed loop:
  // after layout, check the sheet actually sits where the math says and
  // re-measure if not — bounded so scrollbar feedback can't oscillate forever.
  ...
  // Every externally-triggered measure grants the verify loop a fresh retry
  // budget; only the loop's own re-measures consume it.
