...
describe("block-reference task state rendering (GH #518)", () => {
  it.each(MARKERS.flatMap(state => (["md", "org"] as const).map(format => ({ state, format }))))(
    ...
    async ({ state, format }) => {
      ...
      try {
        await vi.waitFor(() => expect(host.querySelector(".block-ref")?.textContent).toBe(`${state} buy milk`));
  ...
  it.each(["md", "org"] as const)("retains literal task words and inline formatting in %s targets", async format => {
    ...
    try {
      await vi.waitFor(() => expect(host.querySelector(".block-ref")?.textContent).toBe("TODO TODO buy milk"));
      expect(host.querySelector(".block-ref em")?.textContent).toBe("milk");
    } finally {
      dispose();
    }
  });
  ...
  it("refreshes unloaded task references after external state/content changes and deletion", async () => {
    ...
    try {
      await vi.waitFor(() => expect(host.querySelector(".block-ref")?.textContent).toBe("WAIT buy milk"));
      bumpDataRev();
      await vi.waitFor(() => expect([...host.querySelectorAll(".block-ref")].map(el => el.textContent))
  ...
  it("clicking the task chip navigates to the source; shift-click opens its sidebar", async () => {
    ...
    try {
      ...
      expect(host.querySelector(".block-ref")?.textContent).toBe("TODO buy milk");
    } finally {
      dispose();
    }
  });
  ...
  it("follows a task-state change on the source block", async () => {
    ...
    try {
      await vi.waitFor(() => expect(host.querySelector(".block-ref .block-marker")?.textContent).toBe("TODO"));
      setRaw(id, `DONE buy milk\nid:: ${id}`);
      await vi.waitFor(() => expect(host.querySelector(".block-ref .block-marker")?.textContent).toBe("DONE"));
      expect(host.querySelector(".block-ref")?.textContent).toBe("DONE buy milk");
      ...
      expect(host.querySelector(".block-ref")?.textContent).toBe("buy milk");
    } finally {
      dispose();
    }
  });
  ...
  it("keeps an explicit alias label instead of the marker", async () => {
    ...
    try {
      await vi.waitFor(() => expect(host.querySelector(".block-ref")?.textContent).toBe("the groceries"));
      expect(host.querySelector(".block-ref .block-marker")).toBeNull();
      setRaw(id, `DONE different groceries\nid:: ${id}`);
      expect(host.querySelector(".block-ref")?.textContent).toBe("the groceries");
      expect(host.querySelector(".block-ref .block-marker")).toBeNull();
    } finally {
      dispose();
    }
  ...
  it("renders no chip for an ordinary non-task reference", async () => {
    ...
    try {
      await vi.waitFor(() => expect(host.querySelector(".block-ref")?.textContent).toBe("just a note"));
      expect(host.querySelector(".block-ref .block-marker")).toBeNull();
    } finally {
      dispose();
    }
