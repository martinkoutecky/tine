...
| SQLite projection | application ID `0x54494e45`; schema 19 | SQLite is disposable. A mismatch must rebuild from authoritative history, never reinterpret rows under a new schema. Schema 19 makes external Logseq UUID claims non-unique and preserves every claimant for bounded application-level classification. |
