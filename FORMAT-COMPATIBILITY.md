...
| SQLite projection | application ID `0x54494e45`; schema 20 | SQLite is disposable. A mismatch must rebuild from authoritative history, never reinterpret rows under a new schema. Schema 20 preserves ambiguous external Logseq UUID claimants, append-only block-ID/home-document and external-UUID introductions, and opaque application-owned causal ownership records for page-name and portable-path point lookups. |
