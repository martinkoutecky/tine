...
| **Selection visibility (dark theme)** | Selected text is nearly invisible on dark. Root cause is a WebKit engine quirk, not the color choice: WebKitGTK rewrites fully-opaque `::selection` backgrounds through a white-page blend, turning the specced `#2b4f74` (would be ~2:1 contrast, on par with VSCode) into an effective ~1.08:1. Alpha-bearing colors are left alone. Fix: dark `--selection-bg: rgba(76,139,245,.5)` / light `rgba(76,139,245,.3)`, a `color-mix` guard in the `::selection` rule so no future theme value re-trips the quirk, and split block-multi-select onto its own subtler `--block-select-bg` (it currently shares the token; OG keeps them distinct). Must verify in the real app — the Chromium screenshot harness doesn't have the WebKit transform. ~1 h. |
| **Tabs: focus the existing tab when navigating** | `g j` duplicates the pinned Journals tab because `navigate()` (`src/router.ts`) never checks whether any tab already shows the destination. The OG Tabs plugin that Tine's pin semantics were modeled on *does* dedupe, and VSCode ("reveal if open"), Firefox ("Switch to Tab"), and Obsidian converge on the same rule: "go to X" focuses an existing view; only explicit new-tab duplicates. Fix: a reuse-if-open branch in `navigate()` for sticky navs (exact route match; middle-click still duplicates), plus one toggle ("Switch to an already-open tab when navigating", default ON). ~2 h incl. router tests. |
...
| **Keyboard-shortcuts help (`?` and `g s`)** | User-suggested. OG: `?` toggles the corner help popup (Handbook, shortcuts, docs, community links); the keymap pane itself is data-driven from the shortcut registry and rebindable. Tine already generates a remappable shortcuts tab in the Settings modal from the single `COMMANDS` table (`src/keybindings.ts`). Tier 1 (~30–60 min): bind `?` (must be written `shift+?` — `eventToChord` sees the shifted character) and `g s` to open Settings directly on that tab. Tier 2 (~½ d): group by scope, move the hardcoded select-mode/editing keys into a static `BUILTIN_KEYS` list next to `COMMANDS` (drift-proof: docs live in the file that defines behavior), port OG's trigger + markdown/org syntax cheat-sheet tables (all map to supported features). |
