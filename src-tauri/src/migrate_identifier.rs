...
// `dev.logseqclaude.app` -> `dev.tine.app` -> `page.tine.app`. The OS app-data dir
// is keyed by the bundle identifier, and on Linux WebKitGTK ALSO stores the
// webview's localStorage / IndexedDB there. localStorage is where the frontend
// keeps the last-opened graph path, the open-tab session, recent pages and the
// right sidebar — so without this, a rename silently orphans not just settings and
// backups but the *entire working state*, and the app boots to the Welcome screen
...
//
// Android is intentionally NOT handled: an applicationId change is a new app at the
// OS level and app-private storage cannot be migrated across it. Tine's Android data
// is minimal (the graph lives in external files), so the only residual is re-picking
// the graph folder once. This shim is desktop-only in effect.

use std::path::Path;
use std::sync::atomic::{AtomicBool, Ordering};

/// The identifier Tine currently ships under (must match tauri.conf.json).
const CURRENT_IDENTIFIER: &str = "page.tine.app";
...
const LEGACY_IDENTIFIERS: &[&str] = &["dev.tine.app", "dev.logseqclaude.app"];
...
mod tests {
    ...
    fn migrates_a_genuine_legacy_dir_when_new_is_absent() {
        ...
        assert!(new.join("SENTINEL").exists(), "localStorage-carrying dir moved");
    ...
    fn prefers_newest_legacy_identifier() {
        ...
        assert_eq!(which, "tine", "newest legacy wins");
        assert!(older.exists(), "older legacy left untouched");
        assert!(!newer.exists(), "newest legacy consumed");
        let _ = std::fs::remove_dir_all(&base);
    }
    ...
    fn falls_back_to_oldest_when_only_it_exists() {
        ...
        assert!(migrate_app_data_dir(&new));
        assert_eq!(std::fs::read_to_string(new.join("WHICH")).unwrap(), "logseqclaude");
        let _ = std::fs::remove_dir_all(&base);
    }
